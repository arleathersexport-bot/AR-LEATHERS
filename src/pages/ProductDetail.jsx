...
export default function ProductDetail() {
  ...
  return (
    <div className="pt-16 md:pt-28 pb-16 md:pb-24 bg-white2">
      <div className="container-x px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10 lg:gap-14">
          ...
          <Reveal>
            <div className="relative w-full">
              <div className="relative rounded-2xl md:rounded-3xl overflow-hidden shadow-luxury w-full" style={{ aspectRatio: "4 / 3" }}>
                <Swiper
                  ...
                  {product.images.map((img, i) => (
                    <SwiperSlide key={i}>
                      <div className="relative w-full h-full group cursor-zoom-in" onClick={() => setLightbox(true)}>
                        ...
                        />
                        <span className="absolute bottom-3 right-3 md:bottom-4 md:right-4 w-8 md:w-10 h-8 md:h-10 rounded-full bg-white2/80 backdrop-blur flex items-center justify-center text-cocoa opacity-0 group-hover:opacity-100 transition-opacity text-sm md:text-base">
                ...
                {product.images.length > 1 && (
                  <>
                    ...
                    {/* Image counter */}
                    <span className="absolute bottom-3 left-3 md:bottom-4 md:left-4 z-10 px-2 md:px-3 py-1 rounded-full bg-charcoal/60 backdrop-blur text-white2 text-xs font-alt tracking-wide">
              ...
              {product.images.length > 1 && (
                <div className="relative mt-3 md:mt-4 px-4 md:px-8">
                  <Swiper
                    modules={[Navigation]}
                  ...
                  >
                    {product.images.map((img, i) => (
                      <SwiperSlide key={i} className="cursor-pointer">
                        <div
                          className={`aspect-[4/3] rounded-lg md:rounded-xl overflow-hidden border-2 transition-colors ${activeIndex === i ? "border-gold" : "border-transparent"}`}
                        >
          ...
          <div className="flex flex-col justify-start">
            ...
            <Reveal delay={0.1}>
              <h1 className="heading-serif text-3xl sm:text-4xl md:text-[2.75rem] mt-4 md:mt-5 leading-tight">{product.name}</h1>
            </Reveal>
            <Reveal delay={0.15}>
              <p className="text-gold font-alt mt-2 text-sm md:text-base">{product.tagline}</p>
            </Reveal>
            <Reveal delay={0.2}>
              <p className="text-charcoal/65 leading-relaxed mt-4 md:mt-6 text-sm md:text-base">{product.overview}</p>
            </Reveal>
              ...
              <div className="mt-6 md:mt-8">
                ...
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3">
                  {product.features.map((f) => (
                    <li key={f} className="flex items-start gap-2 text-xs md:text-sm text-charcoal/65">
                      <HiOutlineCheckCircle className="text-gold shrink-0 mt-0.5 text-sm md:text-base" /> {f}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <Reveal delay={0.3}>
              <div className="mt-6 md:mt-8 flex flex-wrap gap-2">
                {product.applications.map((a) => (
                  <span key={a} className="text-[10px] md:text-xs px-2.5 md:px-3 py-1.5 rounded-full bg-cream text-cocoa font-alt">{a}</span>
                ))}
              </div>
            </Reveal>

            <Reveal delay={0.35}>
              <a href="#inquiry" onClick={(e) => { e.preventDefault(); document.getElementById("inquiry")?.scrollIntoView({ behavior: "smooth" }); }} className="btn-primary mt-6 md:mt-9 inline-flex w-full md:w-auto justify-center md:justify-start">
        ...
        {/* Specifications */}
        <Reveal>
          <div className="mt-12 md:mt-20 grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10">
            <div>
              <h3 className="font-display text-xl md:text-2xl text-charcoal mb-4 md:mb-5">Specifications</h3>
              <div className="rounded-xl md:rounded-2xl overflow-hidden border border-cocoa/10">
                {product.specs.map((s, i) => (
                  <div key={s.label} className={`flex justify-between px-4 md:px-6 py-3 md:py-4 text-xs md:text-sm gap-4 ${i % 2 === 0 ? "bg-cream/40" : "bg-white2"}`}>
            ...
            <div>
              ...
              <div className="bg-cocoa rounded-xl md:rounded-2xl p-6 md:p-8 text-cream/80 leading-relaxed text-sm md:text-base h-full flex items-center">
        ...
        {/* Inquiry CTA */}
        <Reveal>
          <div id="inquiry" className="mt-12 md:mt-20 bg-gradient-to-br from-cream to-camel/40 rounded-2xl md:rounded-3xl p-6 md:p-14 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6">
            <div className="w-full md:w-auto">
              <h3 className="font-display text-xl sm:text-2xl md:text-3xl text-cocoa">Interested in {product.name}?</h3>
              <p className="text-cocoa/60 mt-2 text-sm md:text-base">Send us your requirement and our export team will respond with pricing and lead time.</p>
            </div>
            <button
        ...
        {/* Prev / Next */}
        <div className="mt-12 md:mt-16 flex flex-col sm:flex-row items-center justify-between border-t border-cocoa/10 pt-6 md:pt-8 gap-4">
          <Link to={`/products/${prev.slug}`} className="flex items-center gap-2 md:gap-3 group w-full sm:w-auto sm:max-w-[45%]">
            <HiOutlineArrowLeft className="text-cocoa group-hover:-translate-x-1 transition-transform shrink-0" />
            <div className="min-w-0">
              <p className="text-[10px] uppercase tracking-wide text-charcoal/40 font-alt">Previous</p>
              <p className="font-display text-charcoal group-hover:text-gold transition-colors truncate text-sm md:text-base">{prev.name}</p>
            </div>
          </Link>
          <Link to={`/products/${next.slug}`} className="flex items-center gap-2 md:gap-3 group text-left sm:text-right w-full sm:w-auto sm:max-w-[45%] justify-start sm:justify-end">
            <div className="min-w-0">
              <p className="text-[10px] uppercase tracking-wide text-charcoal/40 font-alt">Next</p>
              <p className="font-display text-charcoal group-hover:text-gold transition-colors truncate text-sm md:text-base">{next.name}</p>
            </div>
            <HiOutlineArrowRight className="text-cocoa group-hover:translate-x-1 transition-transform shrink-0" />
          </Link>
        </div>
            ...
            {relatedFallback.map((p) => (
              <Link key={p.slug} to={`/products/${p.slug}`} className="group block bg-gray2 rounded-xl md:rounded-2xl overflow-hidden hover:shadow-luxury transition-all duration-500">
                ...
                <div className="p-4 md:p-5">
                  <h4 className="font-display text-charcoal group-hover:text-gold transition-colors text-sm md:text-base">{p.name}</h4>
                </div>
              </Link>
            ))}
          </div>
      ...
      <AnimatePresence>
        {lightbox && (
          <motion.div
            className="fixed inset-0 z-[90] bg-cocoa/90 backdrop-blur-md flex items-center justify-center p-4 md:p-6"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <motion.img
              ...
              className="max-h-[85vh] max-w-full rounded-lg md:rounded-2xl shadow-luxury"
            />
            <button className="absolute top-4 md:top-6 right-4 md:right-6 text-white2 text-2xl md:text-3xl" onClick={() => setLightbox(false)}>
              <HiOutlineXMark />
            </button>
          </motion.div>
        )}
